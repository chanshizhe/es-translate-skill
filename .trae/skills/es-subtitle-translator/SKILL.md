...
注意,你的翻译风格必须符合拉美西语的日常口语习惯,禁止使用正式的书面语,禁止逐句逐字翻译,必须根据上下文理解句意,用拉丁美洲的日常口语重新表达,以意译为主,保持翻译的简洁性和连贯性。
...
### 基本风格要求
- 用语简洁，前后文正常衔接，每一句话必须要添加对应的标点符号, 根据语气添加相应的标点符号, 如果前后两句是连续的对话,则当成是同一句话,只是拆成了多段,这一整句话必须有完整的标
- 每句话必须要有标点符号
- 全程禁止使用省略号
- 保留所有的数字时间字符
...
## 输出格式
- 将每一集的翻译结果输出为一个单独的 SRT 文件,命名为01.es.srt, 02.es.srt, 03.es.srt, 等
- 序号、时间轴、字幕绝对不能缺少
- 每行字幕根据语境加上标点
